...
static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Antimony: 3 days
static const int64 nTargetSpacing = 2 * 60; // Antimony: 2 minutes
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc5, 0xb3, 0xda }; // Antimony: increase each by adding 2 to bitcoin's value.
...
void static AntimonyMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running AntimonyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
