...
uint256 hashGenesisBlock("0xc565fbc3090a21b9ae019049036cd80c687bb46659871fe0e0618c20415494f3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Antimony: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Antimony Signed Message:\n";
...
bool AddOrphanTx(const CTransaction & tx) {
    ...
    printf("stored orphan tx %s (mapsz %"
        PRIszu ")\n", hash.ToString().c_str(),
...
bool CTxOut::IsDust() const {
    // Antimony: IsDust() detection disabled, allows any valid dust to be relayed.
...
int CMerkleTx::GetBlocksToMaturity() const {
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY + 20) - GetDepthInMainChain()); // The other 20 blocks for coinbase maturity. 5 is somewhere else in the code.
}
...
int64 static GetBlockValue(int nHeight, int64 nFees) {
    ...
    nSubsidy >>= (nHeight / 420000); // Antimony: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static
const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Antimony: 3 days
static
const int64 nTargetSpacing = 2 * 60; // Antimony: 2 minutes
static
...
unsigned int static GetNextWorkRequired(const CBlockIndex * pindexLast,
    ...
    // Antimony: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    printf("  nActualTimespan = %"
        PRI64d "  before bounds\n", nActualTimespan);
    ...
    printf("nTargetTimespan = %"
        PRI64d "    nActualTimespan = %"
...
void CTransaction::UpdateCoins(CValidationState & state, CCoinsViewCache & inputs, CTxUndo & txundo, int nHeight,
...
bool CBlock::ConnectBlock(CValidationState & state, CBlockIndex * pindex, CCoinsViewCache & view, bool fJustCheck) {
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex -> nHeight, nFees))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"
            PRI64d " vs limit=%"
            PRI64d ")", vtx[0].GetValueOut(), GetBlockValue(pindex -> nHeight, nFees)));
...
bool SetBestChain(CValidationState & state, CBlockIndex * pindexNew) {
    ...
    if (vDisconnect.size() > 0) {
        printf("REORGANIZE: Disconnect %"
            PRIszu " blocks; %s..\n", vDisconnect.size(), pfork -> GetBlockHash().ToString().c_str());
        printf("REORGANIZE: Connect %"
    ...
    if ((pindexNew -> nHeight % 20160) == 0 || (!fIsInitialDownload && (pindexNew -> nHeight % 144) == 0)) {
        const CBlockLocator locator(pindexNew);::SetBestChain(locator);
    }
...
bool CBlock::CheckBlock(CValidationState & state, bool fCheckPOW, bool fCheckMerkleRoot) const {
    ...
    // Antimony: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlockIndex::IsSuperMajority(int minVersion,
    const CBlockIndex * pstart, unsigned int nRequired, unsigned int nToCheck) {
...
bool ProcessBlock(CValidationState & state, CNode * pfrom, CBlock * pblock, CDiskBlockPos * dbp) {
    ...
    for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
        ...
        for (multimap < uint256, CBlock * > ::iterator mi = mapOrphanBlocksByPrev.lower_bound(hashPrev); mi != mapOrphanBlocksByPrev.upper_bound(hashPrev);
            ++mi) {
...
bool LoadBlockIndex() {
    if (fTestNet) {
        ...
        hashGenesisBlock = uint256("0x22667179b9539945f36d869f9d4716be4cbb742436549fa5a58fe846a6f9cbec");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0xe42de3780325f30a8a1753e81bb7a96b411ebcc60dbbe6cfb92e24401ebb3d6f"));
        block.print();
...
void PrintBlockTree() {
    ...
    while (!vStack.empty()) {
        ...
        printf("%d (blk%05u.dat:0x%x)  %s  tx %"
            PRIszu "",
            pindex -> nHeight,
...
bool LoadExternalBlockFile(FILE * fileIn, CDiskBlockPos * dbp) {
    ...
    if (nLoaded > 0)
        printf("Loaded %i blocks from external file in %"
...
unsigned char pchMessageStart[4] = {
    ...
    0xb3,
    0xda
}; // Antimony: increase each by adding 2 to bitcoin's value.
...
bool static ProcessMessage(CNode * pfrom, string strCommand, CDataStream & vRecv) {
    ...
    if (strCommand == "version") {
        ...
        if (vAddr.size() > 1000) {
            ...
            return error("message addr size() = %"
                PRIszu "", vAddr.size());
        }
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++) {
            ...
            if (pfrom -> nSendSize > (SendBufferSize() * 2)) {
                ...
                return error("send buffer size() = %"
                    PRIszu "", pfrom -> nSendSize);
            }
        }
        ...
        if (vInv.size() > MAX_INV_SZ) {
            pfrom -> Misbehaving(20);
            return error("message getdata size() = %"
                PRIszu "", vInv.size());
        }
        ...
        if (tx.AcceptToMemoryPool(state, true, true, & fMissingInputs)) {
            ...
            printf("AcceptToMemoryPool: %s %s : accepted %s (poolsz %"
                PRIszu ")\n",
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
                ...
                if (itByPrev == mapOrphanTransactionsByPrev.end())
                    continue;
                for (set < uint256 > ::iterator mi = itByPrev -> second.begin(); mi != itByPrev -> second.end();
                    ++mi) {
...
class TxPriorityCompare {
    ...
    bool operator()(const TxPriority & a,
        const TxPriority & b) {
        if (byFee) {
            if (a.get < 1 > () == b.get < 1 > ())
...
bool CheckWork(CBlock * pblock, CWallet & wallet, CReserveKey & reservekey) {
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AntimonyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AntimonyMiner(CWallet * pwallet) {
    printf("AntimonyMiner started\n");
    ...
    try {
        loop {
            ...
            printf("Running AntimonyMiner with %"
                PRIszu " transactions in block (%u bytes)\n", pblock -> vtx.size(), ::GetSerializeSize( * pblock, SER_NETWORK, PROTOCOL_VERSION));
        ...
        printf("AntimonyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet * pwallet) {
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads -> create_thread(boost::bind( & AntimonyMiner, pwallet));
}
