...
struct CompareValueOnly {
    bool operator()(const pair < int64, pair <
        const CWalletTx * , unsigned int > > & t1,
            const pair < int64, pair <
                const CWalletTx * , unsigned int > > & t2) const {
...
bool CWallet::AddKeyPubKey(const CKey & secret,
                    ...
                    unsigned int latestEntry = 0; {
                        ...
                        for (TxItems::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
                            ...
                            if (pwtx == & wtx)
                                continue;
                            CAccountingEntry *
                                const pacentry = ( * it).second.second;
                            int64 nSmartTime;
                            if (pwtx) {
...
bool CWallet::AddToWalletIfInvolvingMe(const uint256 & hash,
    const CTransaction & tx,
        const CBlock * pblock, bool fUpdate, bool fFindBlock) {
    {
        LOCK(cs_wallet);
...
static void ApproximateBestSubset(vector < pair < int64, pair <
    const CWalletTx * , unsigned int > > > vValue, int64 nTotalLower, int64 nTargetValue,
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, int nConfMine, int nConfTheirs, vector < COutput > vCoins,
    ...
    pair < int64, pair <
        const CWalletTx * , unsigned int > > coinLowestLarger;
    coinLowestLarger.first = std::numeric_limits < int64 > ::max();
    coinLowestLarger.second.first = NULL;
    vector < pair < int64, pair <
        const CWalletTx * , unsigned int > > > vValue;
    int64 nTotalLower = 0;
    ...
    BOOST_FOREACH(COutput output, vCoins) {
        ...
        pair < int64, pair <
            const CWalletTx * , unsigned int > > coin = make_pair(n, make_pair(pcoin, i));
...
bool CWallet::SelectCoins(int64 nTargetValue, set < pair <
...

    {
        LOCK2(cs_main, cs_wallet); {
            ...
            loop {
                ...
                // Choose coins to use
                set < pair <
                    const CWalletTx * , unsigned int > > setCoins;
                int64 nValueIn = 0;
...
void CWallet::PrintWallet(const CBlock & block) {
    {
        ...
        if (mapWallet.count(block.vtx[0].GetHash())) {
            ...
            printf("    mine:  %d  %d  %"
                PRI64d "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit());
        }
    }
    printf("\n");
}
...
bool CWallet::NewKeyPool() {
    {
        ...
        printf("CWallet::NewKeyPool wrote %"
            PRI64d " new keys\n", nKeys);
    }
    return true;
}
...
bool CWallet::TopUpKeyPool() {
    {
        ...
        // Top up key pool
        unsigned int nTargetSize = max(GetArg("-keypool", 100), 0 LL);
