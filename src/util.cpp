...
#ifndef NOMINMAX
#define NOMINMAX
#endif#include <io.h> /* for _commit */
...
void locking_callback(int mode, int i,
...
string FormatMoney(int64 n, bool fPlus) {
    ...
    string str = strprintf("%"
        PRI64d ".%08"
        PRI64d, quotient, remainder);

    // Right-trim excess zeros before the decimal point:
    int nTrim = 0;
    for (int i = str.size() - 1;
        (str[i] == '0' && isdigit(str[i - 2])); --i)
        ++nTrim;
    if (nTrim)
...
void AddTimeData(const CNetAddr & ip, int64 nTime) {
    ...
    printf("Added time data, samples %d, offset %+"
        PRI64d " (%+"
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1) {
        ...
        if (abs64(nMedian) < 35 * 60) // Antimony: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        } else {
            nTimeOffset = 0;
...
void seed_insecure_rand(bool fDeterministic) {
    ...
    if (fDeterministic) {
        ...
        } while (tmp == 0 || tmp == 0x9068ffff U);
        insecure_rand_Rz = tmp;
        do {
            RAND_bytes((unsigned char * ) & tmp, 4);
        } while (tmp == 0 || tmp == 0x464fffff U);
