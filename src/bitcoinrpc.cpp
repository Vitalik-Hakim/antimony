...
static inline unsigned short GetDefaultRPCPort() {
    return GetBoolArg("-testnet", false) ? 12334 : 2334;
}
...
string CRPCTable::help(string strCommand) const {
    ...
    for (map < string,
        const CRPCCommand * > ::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi) {
...
static
const CRPCCommand vRPCCommands[] = { //  name                      actor (function)         okSafeMode threadSafe reqWallet
    //  ------------------------  -----------------------  ---------- ---------- ---------
    {
        "help",
        & help,
...
const CRPCCommand * CRPCTable::operator[](string name) const {
    map < string,
        const CRPCCommand * > ::const_iterator it = mapCommands.find(name);
...
static string HTTPReply(int nStatus,
    const string & strMsg, bool keepalive) {
...
template < typename Protocol >
    class SSLIOStreamDevice: public iostreams::device < iostreams::bidirectional > {
        public: SSLIOStreamDevice(asio::ssl::stream < typename Protocol::socket > & streamIn, bool fUseSSLIn): stream(streamIn) {
            fUseSSL = fUseSSLIn;
...
template < typename Protocol >
    class AcceptedConnectionImpl: public AcceptedConnection {
        ...
        private: SSLIOStreamDevice < Protocol > _d;
...
void StartRPCThreads() {
    ...
    if ((mapArgs["-rpcpassword"] == "") ||
        ...
        string strWhatAmI = "To use antimonyd";
